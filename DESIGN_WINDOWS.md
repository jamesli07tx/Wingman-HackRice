...
5. **Git flow:** Windows creates the shared GitHub repo at hour 0 and pushes the skeleton first. Mac clones, then commits only inside `glassbridge/`. Both push to `main` freely — disjoint paths mean no conflicts. (Offline fallback: the Mac's `glassbridge/` folder is copied into the repo root by USB/AirDrop; same result.)
