...
- lines: 3 to 5 bullets, each at most 40 characters INCLUDING its closing period, each a complete short sentence ending with a period, each useful to a student standing at the booth right now. No leading bullet character — the display adds it. When hiring roles are known, the first line starts with "Hiring: " and names them. Priority after that: the technologies or domains a new hire would touch; university-recruiting facts (campus programs, deadlines, return-offer or co-op norms); one concrete recent development. One fact per line, each standing alone.
...
- No emoji, no markdown, no line that is only a slogan. Every line ends with exactly one period.
