...
export function OverridePicker() {
  ...
  return (
    <Section
      title="Override"
      hint="Force a company onto the lens — bypasses the gate, the cooldown and the confidence floor."
    >
      <Input
        value={query}
        onChange={(e) => setQuery(e.target.value)}
        placeholder="Search companies… (e.g. stri)"
        aria-label="Search companies"
      />
      {searching ? <p className="mt-2 text-[11px] text-zinc-600">Searching…</p> : null}
      {results.length > 0 ? (
        <ul className="mt-2 max-h-56 space-y-1 overflow-y-auto scroll-thin">
          {results.map((r) => (
            <li key={r.companyId}>
              <div className="flex items-center justify-between gap-2 rounded-lg border border-[var(--color-edge)] px-3 py-2">
                ...
                <Button variant="primary" onClick={() => void force(r)}>
                  Force
                </Button>
              </div>
            </li>
